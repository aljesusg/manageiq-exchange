...
RSpec.describe Providers::GithubManager, type: :model do
  ...
  it 'user' do
    user_obj = {
      login:               'user',
      id:                  3019213,
      avatar_url:          'https://avatars2.githubusercontent.com/u/323233?v=4',
      gravatar_id:         '',
      ...
      email:               'user@manageiq-exchange.com',
      hireable:            true,
      bio:                 nil,
      public_repos:        45,
      public_gists:        0,
    ...
    VCR.use_cassette("sessions/session-new-user-good-code",:decode_compressed_response => true, :record => :none) do
      VCR.use_cassette("providers/github/get_user",:decode_compressed_response => true, :record => :none) do
        subject.exchange_code_for_token!(code)
        result  = subject.user
        expect(result).to be_kind_of Sawyer::Resource
        expect(result.to_hash).to eq( user_obj )
        expect(subject.github_access.user.to_hash).to eq( user_obj )
      end
    end
  end
